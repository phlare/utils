...
Tree.prototype = {
  addNode: function(key, data, parent) {
  ...
  // helper function for find to make it easy for user to call
  ...
  // this function will likely be overwritten in a specialized tree
  findNodeDepthFirst: function(key, node, parent) {
  ...
  // my current implementation of d3 code is destructive, so until i fix that,
  // I have to create a fresh 'display' version of the tree and print that.
  createDisplayData: function(root) {
    var displayData = {
      root: root.data,
      key: root.key,
      children: []
  ...
  // this was just kludged into working shape by adapting some of the code here
  ...
  // see Tree.display()
  displayTree: function(root, method) {
    ...
    if (method === 'd3' && typeof d3 === 'object') {
      ...
      // swap children and _children on click
      // this will expand/collapse that node
      function click(d) {
        if (d.children) {
